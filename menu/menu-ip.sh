...
APIGIT=$(cat /etc/anggun/github/api)
EMAILGIT=$(cat /etc/anggun/github/email)
USERGIT=$(cat /etc/anggun/github/username)
...
if [[ -f /etc/anggun/github/api && -f /etc/aggun/github/email && /etc/anggun/github/username ]]; then
   rec="OK"
else
    mkdir /etc/anggun/github > /dev/null 2>&1
fi
...
function resetipvps(){
clear
rm -f /etc/anggun/github/email
rm -f /etc/anggun/github/username
rm -f /etc/anggun/github/api
rm -f /etc/anggun/github/gitstat
echo "OFF" > /etc/anggun/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/arismaramar/permission/main/ipmini | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
if [ -f "$GITREQ" ]; then
    ...
    mkdir /etc/anggun/github
    touch /etc/anggun/github/gitstat
    echo "OFF" > /etc/anggun/github/gitstat
fi

stst1=$(cat /etc/anggun/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1 ${NC} ${COLBG1}               ${WH}• REGISTER IPVPS •              ${NC} $COLOR1 $NC"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── ${WH}BY${NC} ${COLOR1}───────────────────────┐${NC}"
echo -e "$COLOR1 ${NC}                ${WH}•   ANNGUN @ 2023 •${NC}                 $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e ""
read -n 1 -s -r -p "   Press any key to Set API"
setapi
fi
stst=$(cat /etc/anggun/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
